...
@endphp
<script type="text/template" id="matReqRowTpl-{{ $fault->id }}">
        ...
        <div class="col-xl-5 col-lg-5 col-md-5 col-sm-12">
          <div class="row gx-2 gy-1 align-items-end">
            <div class="col-md-6 col-sm-12">
              ...
              <select class="form-select form-select-sm mat-select" name="items[ROW_IDX][material_id]" data-row="ROW_IDX" data-fault="{{ $fault->id }}">
                <option value="">— Select SKU —</option>
                @foreach($matList as $m)
                  <option value="{{ $m->id }}"
                    data-name="{{ e($m->name) }}"
                    data-unit="{{ e($m->unit) }}"
                    data-stock="{{ $m->quantity_on_hand }}">
                    {{ $m->name }}@if($m->sku) ({{ $m->sku }})@endif @if($m->category) · {{ $m->category }}@endif
                  </option>
                @endforeach
...
<div class="modal custom-modal fade" id="requestMaterialCreateModal-{{ $fault->id }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="requestMaterialCreateModalLabel-{{ $fault->id }}" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <div>
          <h5 class="modal-title mb-0" id="requestMaterialCreateModalLabel-{{ $fault->id }}">
          @if($isEdit)
        ...
        <div class="text-muted small mt-1">
          @if($isEdit)
            Editing existing request <span class="font-monospace">{{ $editingMr->request_number }}</span> — your previous values are pre-loaded below. Changes replace the open request.
          @else
        ...
        @if($isEdit && $mrBadge)
        <div class="mt-2">
          <x-status-badge :label="$mrBadge['label']" :color="$mrBadge['color']" :soft="true" />
        </div>
        @endif
        </div>
      ...
      <form action="{{ route('stores.store') }}" method="POST" id="matReqForm-{{ $fault->id }}" class="mat-req-form">
        @csrf
        <input type="hidden" name="fault_id" value="{{ $fault->id }}">
        @if($isEdit)
        <input type="hidden" name="_replace_mr_id" value="{{ $editingMr->id }}">
        @endif
        ...
        <div class="modal-body">
          <div class="mr-fault-note mb-4 d-flex align-items-start gap-2">
            ...
            <div>
              Requesting for <strong>{{ $faultRef }}</strong>. Include UTP cable (meters), RJ45 connectors, splice protectors, etc. Your name will be logged automatically.
            </div>
          </div>
          ...
          <div class="mr-section-card mb-4">
            ...
            <textarea name="technician_note" class="form-control form-control-sm mt-2" rows="2" placeholder="Any additional context or special request, e.g. Urgent same-day dispatch, outdoor rated only...">{{ old('technician_note', $isEdit ? e($editingMr->technician_note ?? '') : '') }}</textarea>
          </div>
          ...
          <div class="d-flex justify-content-between align-items-center mb-3 pe-1">
            ...
            <button type="button" class="btn btn-sm btn-outline-primary rounded-pill add-mat-row-btn" data-fault="{{ $fault->id }}" id="addRowBtn-{{ $fault->id }}" style="border-radius:9999px;padding:.35rem .85rem;font-weight:600;font-size:.8rem;">
              <i class="fas fa-plus me-1"></i>Add Row
            </button>
          </div>

          <div class="mat-req-items mat-req-items-{{ $fault->id }}">
            @for($r = 0; $r < $rowCount; $r++)
              {!! renderMatReqRow_editAware($r, $matList, $editItems[$r] ?? null) !!}
            @endfor
          </div>
          ...
          <div class="mt-4">
            ...
            @foreach($pendingReqs as $pr)
              @if($isEdit && (int)$pr->id === (int)$editingMr->id) @continue @endif
              <div class="card mb-2 border-warning-subtle">
                <div class="card-body py-2 px-3">
                  <div class="d-flex justify-content-between align-items-center mb-1">
                    <strong class="small">{{ $pr->request_number }}</strong>
                    <span class="badge" style="background:#F59E0B22;color:#B45309;">{{ $pr->statusBadge()['label'] }}</span>
                  </div>
                  <ul class="list-unstyled small mb-0">
                    @foreach($pr->items as $itm)
                      <li class="d-flex justify-content-between py-1 border-top">
                        <span>{{ $itm->material_name }}</span>
                        <span class="text-muted">{{ $itm->quantity_requested }} {{ $itm->unit }}</span>
                      </li>
                    @endforeach
                  </ul>
                </div>
        ...
        <div class="modal-footer fault-modal-footer">
          ...
          <button type="submit" class="btn btn-primary btn-sm rounded-pill mat-req-submit" data-fault="{{ $fault->id }}">
            @if($isEdit)
