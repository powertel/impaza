...
import { getResolvedFaults } from '../services/api';
import { theme } from '../styles/theme';
import { Feather } from '@expo/vector-icons';

export default function ResolvedFaultsScreen() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const items = await getResolvedFaults();
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity style={styles.card} onPress={() => navigation.navigate('FaultDetail', { id: item.id })}>
      <View style={[styles.priorityBar, { backgroundColor: theme.colors.success }]} />
      <View style={styles.cardContent}>
        <View style={styles.cardHeader}>
          <Text style={styles.customerName}>{item.customer || 'N/A'}</Text>
          <Text style={styles.date}>{new Date(item.updated_at || item.created_at).toLocaleDateString()}</Text>
        </View>
        <Text style={styles.reference}>Ref: {item.fault_ref_number}</Text>
        <Text style={styles.assignedTo}>Resolved by: {item.assignedToName || 'Unknown'}</Text>
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top", "left", "right"]}>
      <View style={styles.header}>
        <Text style={styles.headerTitle}>Resolved Faults</Text>
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.empty}>No resolved faults found.</Text>}
      />
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  assignedTo: { fontSize: theme.fontSizes.sm, color: theme.colors.dark, marginBottom: 8 },
  tag: { backgroundColor: '#D1FAE5', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 12, alignSelf: 'flex-start' },
