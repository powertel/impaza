...
export default function UnassignedFaultsScreen() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const items = await getUnassignedFaults();
  ...
  const renderItem = ({ item }) => (
    <FaultCard 
      item={item} 
      onPress={() => navigation.navigate('AssignFault', { fault: item, mode: 'assign' })} 
    />
  );
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top", "left", "right"]}>
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.empty}>No unassigned faults found.</Text>}
      />
    </SafeAreaView>
  );
}
