...
export default function RectifiedFaultsScreen() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const items = await getRectifiedFaults();
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity style={styles.card} onPress={() => navigation.navigate('FaultDetail', { id: item.id })}>
      ...
      <View style={styles.cardContent}>
        ...
        <Text style={styles.assignedTo}>Tech: {item.assignedToName || 'Unknown'}</Text>
        <View style={styles.tag}>
           <Text style={styles.tagText}>Rectified</Text>
        </View>
      </View>
    </TouchableOpacity>
  );
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top", "left", "right"]}>
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.empty}>No rectified faults found.</Text>}
      />
    </SafeAreaView>
  );
}
