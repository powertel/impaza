...
export default function ReferredFaultsScreen() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const items = await getReferredFaults();
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity style={styles.card} onPress={() => navigation.navigate('FaultDetail', { id: item.id })}>
      <View style={[styles.priorityBar, { backgroundColor: theme.colors.warning }]} />
      <View style={styles.cardContent}>
        ...
        <Text style={styles.assignedTo}>Referred by: {item.assignedToName || 'Unknown'}</Text>
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top", "left", "right"]}>
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.empty}>No referred faults found.</Text>}
      />
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  tag: { backgroundColor: '#FEF3C7', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 12, alignSelf: 'flex-start' },
